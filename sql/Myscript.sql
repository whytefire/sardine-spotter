...
CREATE TABLE Sightings (
    ...
    -- 'sardine_sighting' | 'community_notification'
    is_active       BIT             NOT NULL DEFAULT 1,
    created_at      DATETIME        NOT NULL DEFAULT GETDATE()
);
GO
...
CREATE TABLE FeedPreferences (
    ...
    show_community  BIT             NOT NULL DEFAULT 1,
...
WHERE parent_object_id = OBJECT_ID('FeedPreferences') AND parent_column_id = (
    SELECT column_id FROM sys.columns WHERE object_id = OBJECT_ID('FeedPreferences') AND name = 'show_community'
);

EXEC('ALTER TABLE FeedPreferences DROP CONSTRAINT ' + @constraintName);
ALTER TABLE FeedPreferences DROP COLUMN show_community;
GO